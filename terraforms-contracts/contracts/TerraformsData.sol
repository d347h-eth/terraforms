...
contract TerraformsData is TerraformsDataStorage, Ownable {
        ...
        returns (int)
    {
        ...
        // Note: altered of the original code to avoid "stack too deep" error
        int x = refXY + int(tile % levelDimensions[level]) * stepsize * scale;
        int y = refXY + int(tile / levelDimensions[level]) * stepsize * scale;
        int z = int((level+1) * TOKEN_DIMS * 2 + (seed*TOKEN_DIMS)) * STEP * scale;

        int result = perlinNoise.noise3d(x, y, z);
