// Code for token 5391
// https://tokens.mathcastles.xyz/terraforms/token-html/5391
...
setInterval(() => {
	for (let e = 0; e < 32; e++)
		for (let t = 0; t < 32; t++) {
			...
			0 == MODE ? r.h >
			6 - RESOURCE &&
			(r.textContent = mainSet[Math.floor(.25 * airship + (r.h + .5 * e + .1 * DIRECTION * t)) % mainSet.length]): (isDaydream || isTerraformed) &&
			(0 == r.h ? SEED < 8e3 ? r.textContent = mainSet[Math.floor(airship / 1e3 + .05 * e + .005 * t) % mainSet.length] : r.textContent = mainSet[Math.floor(airship / 2 + .05 * e) % mainSet.length] : (r.textContent = charSet[Math.floor(airship / speedFac + e + r.h) % charSet.length], SEED > 5e3 && Math.random() < .005 &&
				(r.style.fontSize = `${3+airship%34}px`))), 'j' != r.originalClass &&
